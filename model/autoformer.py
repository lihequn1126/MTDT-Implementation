...
parser.add_argument('--output_dir', type=str, default=r"../result/baoxing_autoformer", help='Output directory')
...
SEQ_LEN = 96        
LABEL_LEN = 48      # Label length (Autoformer 重要参数)
PRED_LEN = 96       
POINTS_PER_DAY = 96 
...
d_ff = 128          # 原 2048 -> 改为 128
n_heads = 4         # 原 8 -> 改为 4
e_layers = 2        
d_layers = 1        
dropout = 0.05      # 稍微降低 dropout
...
class Autoformer(nn.Module):
    ...
    def forward(self, x_enc):
        # 1. Decomposition of x_enc
        seasonal_init, trend_init = self.decomp(x_enc)
        
        # 2. Decoder Input Construction (Standard Autoformer logic)
        # Trend: [Past_Trend_Slice, Repeat_Mean]
        ...
        # Seasonal: [Past_Seasonal_Slice, Zeros]
        ...
        # x_dec: concat for embedding
        x_dec = seasonal_init # In official autoformer, x_dec input for embedding is often just the seasonal part placeholder
...
def train_and_evaluate():
    ...
    print(f"Autoformer Parameters: {sum(p.numel() for p in model.parameters())}")
    
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    
    print("Training Autoformer (Lite)...")
